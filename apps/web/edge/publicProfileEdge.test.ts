...
describe("Cloudflare social-card proxy", () => {
  ...
  it("isolates the Share Card cache and sends the approved variant to the renderer", async () => {
    ...
    const fetcher = vi.fn(async (input: URL | RequestInfo) => {
      ...
      return new Response(jpeg, {
        headers: {
          ...
          "X-Social-Card-Template-Version": "pet-share-card-v3",
        },
        status: 200,
      });
    });
    ...
    expect(cache.keys()).toEqual(
      expect.arrayContaining([
        expect.stringContaining("variant=share-card&template=pet-share-card-v3"),
  ...
  it("isolates birthday and adoption caches by variant, template, and Malaysia day", async () => {
    ...
    try {
      ...
      expect(cache.keys()).toEqual(
        expect.arrayContaining([
          expect.stringContaining("variant=birthday&template=pet-birthday-card-v3&day=2026-08-17"),
          expect.stringContaining("variant=adoption&template=pet-adoption-card-v3&day=2026-08-17"),
        ])
      );
    } finally {
      vi.useRealTimers();
  ...
  it("revalidates profile visibility before consulting an old image cache", async () => {
    ...
    await cache.value.put(
      new Request(
        `https://mypetlink.com.my/social/pets/${newPet.publicSlug}.jpg?v=${newPet.publicProfileVersion}&variant=share-card&template=pet-share-card-v3`
      ),
