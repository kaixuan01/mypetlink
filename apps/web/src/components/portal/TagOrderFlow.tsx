...
import { getDeliveryQuoteErrorMessage, getOwnerOrderFieldErrors, isDeliveryUnavailableError, isOutOfStockError } from "@/services/ownerOrderErrors";
...
type DeliveryQuoteState =
  ...
  // "out-of-stock" is deliberately separate from "failed": an inventory
  // conflict must not offer a delivery retry that can never succeed.
  | { status: "unavailable" | "failed" | "out-of-stock"; fingerprint: string; message: string };
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      getDeliveryQuote(delivery.stateCode!, cartQuoteItems, controller.signal)
        ...
        .catch((caught) => {
          if (!controller.signal.aborted && requestNumber === quoteRequestRef.current) setQuoteState({ status: isOutOfStockError(caught) ? "out-of-stock" : isDeliveryUnavailableError(caught) ? "unavailable" : "failed", fingerprint: quoteFingerprint, message: getDeliveryQuoteErrorMessage(caught) });
        });
    }, 250);
