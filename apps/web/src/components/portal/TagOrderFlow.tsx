    ...
    return (
      <section className="rounded-[1.75rem] border border-pet-mint bg-[#e8f8f0] p-6 shadow-sm">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          ...
          <SummaryItem label="Product" value={createdOrder.productName ?? createdOrder.tagType} />
...
function ConfirmationStep({ choice, pet, delivery }: { choice: CatalogChoice; pet: Pet; delivery: DeliveryDetails }) {
  ...
  return <StepShell title="Confirm order" description="Review the product, delivery details, and current price before payment."><div className="grid gap-3 sm:grid-cols-2"><SummaryItem label="Product" value={product.name} /><SummaryItem label="SKU" value={variant.sku} /><SummaryItem label="Variant" value={`${variant.name} · ${variant.tagVariant}`} /><SummaryItem label="Selected pet" value={pet.name} /><SummaryItem label="Tag price" value={formatCatalogPrice(variant.price.finalPrice, variant.price.currency)} /><SummaryItem label="Delivery fee" value={paymentConfig.deliveryFee} /><SummaryItem label="Total" value={formatCatalogPrice(variant.price.finalPrice, variant.price.currency)} /><SummaryItem label="Delivery address" value={formatDeliverySummary(delivery)} /></div>{variant.price.discountAmount > 0 ? <p className="mt-4 rounded-xl bg-[#e8f8f0] p-4 text-sm font-bold text-pet-sage">{variant.price.promotionLabel ?? variant.price.promotionName}: you save {formatCatalogPrice(variant.price.discountAmount, variant.price.currency)}.</p> : null}<p className="mt-4 rounded-xl bg-pet-cream p-4 text-sm leading-6 text-pet-muted">The final amount is recalculated when you place the order. After that, pay with the merchant QR and submit your payment proof for verification.</p></StepShell>;
}
