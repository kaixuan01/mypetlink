...
export function PetLifecycleActions({
  ...
  const [action, setAction] = useState<PetLifecycleAction | null>(null);
  const [message, setMessage] = useState("");
  const copy = action
    ? getPetLifecycleConfirmation(action, pet.name)
    : null;
  const actions = getPetLifecycleActions(pet);
  ...
  function executeAction() {
    ...
    const execution = getPetLifecycleActionExecution(action);
    if (execution.kind === "restore") {
      void restore();
      return;
    }

    void updateLifecycle(execution.status);
  }
