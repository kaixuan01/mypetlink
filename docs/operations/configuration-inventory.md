...
| `RateLimiting:Social:*` (`Follow`, `Like`, `Comment`, `Withdraw`, `MomentCreate`, `Search`, `HandleAvailability`, `ProfileMutation`) | `SocialRateLimitingOptions` | Abuse protection for social endpoints, partitioned by user id. Comments default to 20 per 10 minutes. **In-process: per application instance, not a global limit.** See `docs/deployment/environment-variables.md` for the conditions that require moving to a distributed or edge limiter. |
