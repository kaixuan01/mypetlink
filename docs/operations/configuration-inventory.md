...
| `RateLimiting:Social:*` (`Follow`, `Like`, `Comment`, `CollaborationInvite`, `Report`, `Withdraw`, `MomentCreate`, `Search`, `HandleAvailability`, `ProfileMutation`) | `SocialRateLimitingOptions` | Abuse protection for social endpoints, partitioned by user id. Comments default to 20 per 10 minutes; collaboration invitations to 20 per hour; Community reports to 10 per hour. The collaboration product rules (3 live households per Moment, 3 pending per household pair, 30 invitations a day, 14-day expiry) are approved V1 constants in `MomentCollaborationService`, not settings. **In-process: per application instance, not a global limit.** See `docs/deployment/environment-variables.md` for the conditions that require moving to a distributed or edge limiter. |
| `Social:HandleRenameCooldownDays`, `Social:ReleasedHandleHoldDays` | `SocialOptions` | Handle lifecycle protection: stops name-cycling impersonation, and stops a released handle resolving to a stranger while shared links are still live. Deployment-owned — these protect links and slow abuse rather than express a business decision, so they are deliberately not Admin-editable. |
| `Social:ThumbnailMaxEdgePixels`, `Social:ThumbnailJpegQuality`, `Social:ThumbnailGenerationEnabled` | `SocialOptions` | Image derivative pipeline. The enable flag is a kill switch only: readers already fall back to the original whenever a thumbnail is absent, so disabling it changes no read path. |
...
| Sales commission policy | `CommissionRules` | `/admin/merchant-sales?tab=commissions` | Effective-dated direct-retail percentage, reseller acquisition tier, and reseller repeat-term rules with optional salesperson scope. Salesperson-specific overrides precede global rules. Overlap validation, typed database checks, audit, and `RowVersion` apply. Direct retail resolves per order; reseller repeat terms are frozen once at activation. Missing activation terms fail payment confirmation closed before financial state changes. |
| Admin roles and permissions | `AdminRoles`, `AdminRoleCapabilities`, `AdminUserRoles` | `/admin/access/roles`, `/admin/access/users` | Which internal roles exist, what each grants, and who holds them. Permission keys are validated against the application's capability catalogue — a key that is not in it is refused on save and ignored when access is resolved, so no request payload can invent permission. Built-in roles cannot be deleted and the all-access role's permissions cannot be edited. Audited, `RowVersion` on `AdminRoles` and `AdminUsers`. Deny by default: an administrator with no roles has no permissions. See [`../architecture/admin-access-management.md`](../architecture/admin-access-management.md). |
...
| **Held back** (`SuppressedCount`) | Recorded while the template was off, or while an operational recipient was unavailable. Never sends automatically. Only current-boundary payment-proof review alerts suppressed exactly for an unavailable recipient have an audited, same-row recovery action. |
...
| Commission calculation/history | `SalesCommissions` | Source, type, seller/rule snapshots, base, amount, payout, and reversal history are system-generated. Admin may only use the explicit payout/reversal lifecycle. |
| `EmailOutbox` status, attempts, lease | `EmailOutboxDispatcher` | Machine-managed. Template-disabled and historical `Suppressed` rows are never dispatchable. The sole recovery exception is the audited Admin action for in-boundary `AdminPaymentProofSubmitted` rows suppressed exactly as `OperationsRecipientUnavailable`; it reuses the same row and is idempotent. |
| `__EFMigrationsHistory` | EF Core | Migration state |
| `AuditLogs` | `AuditLogService` | Append-only |
| `AdminUsers.Role` | Legacy | Retained read-only for one rollback window. Nothing authorizes on it; access comes from `AdminUserRoles`. |
...
| Email | Operations recipient configured/missing | safety validation of `Email:OperationsRecipient` (never the value) |
...
When global delivery or the Payment proof review alert template is enabled and
the operations recipient is invalid, the response includes a high-severity
operational warning. Proof submission remains available and the alert is held
back. No secret, address, host, credential, endpoint, or provider error detail
is exposed.
