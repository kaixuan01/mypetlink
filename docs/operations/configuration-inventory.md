...
| `RateLimiting:Social:*` (`Follow`, `Like`, `Withdraw`, `MomentCreate`, `Search`, `HandleAvailability`, `ProfileMutation`) | `SocialRateLimitingOptions` | Abuse protection for social endpoints, partitioned by user id. **In-process: per application instance, not a global limit.** See `docs/deployment/environment-variables.md` for the conditions that require moving to a distributed or edge limiter. |
