...
| Sales commission policy | `CommissionRules` | `/admin/merchant-sales?tab=commissions` | Effective-dated direct-retail percentage rules with optional salesperson scope and quantity bounds. Salesperson-specific overrides precede global rules. Overlap validation, audit, and `RowVersion` apply; missing coverage fails payment approval closed before financial state changes. |
