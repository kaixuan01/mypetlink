...
| `NEXT_PUBLIC_SHARE_CARDS_ENABLED` | No | Owner-facing Share Card preview, native share, save, and copy actions. Defaults to `false`; enable only after the production edge route is verified. The public image route remains available when this UI flag is off. | `false` |
...
| `Storage__Provider` | `Storage:Provider` | No | Set to `CloudflareR2` in production. Current `MediaService` always uses R2; this value enables R2 startup validation and accurate operational status. The default `Local` does not provide a working API media fallback. |
