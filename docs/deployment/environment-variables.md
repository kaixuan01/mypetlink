...
| `NEXT_PUBLIC_PUBLIC_PROFILES_ENABLED` | No | Owner-facing Share Profile sharing and management UI (`/p/*`; the UI still labels it "Public Profile"). Defaults to `true`; keep enabled for the current release. Public routes remain compatible if the owner UI is temporarily hidden. | `true` |
