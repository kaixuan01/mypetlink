...
- Generate tag codes (1–500 per batch): creates **Unclaimed** stock — a TagCode with no pet and no owner — with an automatic `MPL-BAT-yyMMddHHmmss-NNNN` reference. The 500-unit request ceiling matches the existing bulk-inventory operational limit and comfortably supports normal production runs such as 300 units. The API rejects quantities outside this range; it never truncates them. Customers activate by scanning the printed QR at `/q/{tagCode}` → link to a pet → Active. Legacy printed tags at `/t/{tagCode}` still activate; NFC never offers first-time activation.
