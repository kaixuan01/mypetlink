...
- Generate tag codes (1–50 per batch): creates **Unclaimed** stock — a TagCode with no pet and no owner — with an automatic `MPL-BAT-yyMMddHHmmss-NNNN` reference. Customers activate via scan/tap on `/t/{tagCode}` → link to a pet → Active.
